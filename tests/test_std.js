...
function test_popen()
{
    ...
    /* execute a shell command to read the file */
    f = std.popen((os.platform == "win32" ? "type " : "cat ") + fname, "r");
    str = f.readAsString();
    f.close();
...
function test_os()
{
    ...
    if (typeof os.exec == "function") {
        ...
        assert(typeof os.isatty(0), "boolean", `isatty(STDIN)`);
    }
    ...
    assert(err, 0);
    if (typeof os.S_IFMT != "undefined")
        assert(st.mode & os.S_IFMT, os.S_IFREG);
    assert(st.mtime, fdate);

    if (typeof os.symlink == "function") {
